...
def test_current_user_success(
    ...
    with patch.object(
        user_module,
        "decode_access_token",
        return_value=JwtPayload(
            sub=user_stub.id,
            org=user_stub.organization_id,
            login=user_stub.login,
            pv=user_stub.perm_version,
            iat=0,
            exp=0,
            jti="x",
        ),
    ), patch.object(user_module, "fetch_permissions", return_value={}):
...
def test_current_user_perm_changed(
    client: TestClient, session_mock: AsyncMock, user_stub: User
):
...
def test_current_user_not_found(
    ...
    with patch.object(
        ...
        "decode_access_token",
        return_value=JwtPayload(
            sub=1, org="org", login="l", pv=1, iat=0, exp=0, jti="x"
        ),
    ):
        resp = client.get("/user", headers={"Authorization": "Bearer token"})
